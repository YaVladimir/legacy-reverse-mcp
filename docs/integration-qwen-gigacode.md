...
3. **(Рекомендуется) Сгенерированы описания** — смысловой слой (Phase 2), чтобы
   ...
   **Рескан (`scan --force`):** импортированные описания (шаг 4) восстанавливаются
   в пересобранный индекс автоматически в конце скана (устаревшие — где класс
   структурно изменился — пропускаются). Описания из собственного LLM-прохода
   `describe` и саммари уровня module/project не восстанавливаются — после рескана
   запустите `describe` повторно (дёшево: кеш-попадания не тратят LLM-бюджет).
